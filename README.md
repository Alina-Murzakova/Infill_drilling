#  Infill drilling 
## Авто-подбор зон и кандидатов для уплотняющего бурения (АВНС)
...
описаны в [литературном обзоре нашей команды](https://proneft.elpub.ru/jour/article/download/536/532).  
Этот обзор стал отправной точкой при создании и проектировании системы «АВНС».
...
* Построение нормализованных карт:
  ...
  * **Оценка рисков** — учет обводненности и падения давления. 
  * **Индекс возможностей бурения (OI)** = комбинация трех оценок.
> **Результат:** набор нормализованных карт для поиска перспективных зон.
...
> Методологический подход к размещению проектных скважин можно изучить в отдельном репозитории:
> [Init_project_wells](https://github.com/Alina-Murzakova/Init_project_wells)
...
* Построение **карт ОИЗ и обводнённости** (при отсутствии) - библиотека [`reservoir-maps`](https://github.com/Alina-Murzakova/reservoir-maps).:
  * теория Баклея–Леверетта; 
  * учёт взаимодействия скважин;
  * учет материального баланса.

* Поддержка **history matching** для калибровки ОФП (опционально - `switch_adaptation_relative_permeability`).
...
`local_parameters.txt` содержит все необходимые дополнительные свойства и параметры, которые необходимо заполнить пользователю, 
либо которые имеют значения по умолчанию.
...
По итогам разработки системы «АВНС» планируется подготовка отдельной статьи, посвящённой методике и примерам её применения.