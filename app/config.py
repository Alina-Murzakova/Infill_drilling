...
sample_data_wells = pd.DataFrame(columns=[
    ...
    'init_water_cut',  # [16] стартовая обводненность, д.ед.
    'init_water_cut_TR',  # [17] стартовая обводненность ТР, д.ед.
    ...
    'MULTILINESTRING_geo',  # [56] MULTILINESTRING для МЗС в географических координатах, для остальных свои геометрии
    'POINT_T1_pix',  # [57] тип POINT для точки T1 в пикселях
    'POINT_T3_pix',  # [58] тип POINT для точки T3 в пикселях
    'LINESTRING_pix',  # [59] LINESTRING для ствола скважины в пикселях
    'MULTILINESTRING_pix',  # [60] MULTILINESTRING для МЗС в пикселях, для остальных свои геометрии

    # frac_params:
    'xfr',  # [61] полудлина трещины ГРП, м
    'w_f',  # [62] раскрытие трещины ГРП, мм
    'FracCount',  # [63] количество стадий ГРП, шт
    'length_FracStage',  # [64] шаг стадии для определения количества стадий МГРП, м

    # Дополнительные столбцы:
    'well_number_digit',  # [65] номер скважины без доп части (числовая часть)
    'type_wellbore',  # [66] тип ствола скважины (Материнский ствол/ЗБС/МЗС)
    'first_well_date',  # [67] дата начала эксплуатации скважины
    'number_wellbore',  # [68] порядковый номер ствола скважины (на случай ЗБС)
    'first_production_date',  # [69] дата первой добычи нефти
    'continuous_work_months',  # [70] длительность последнего периода работы, мес
    'priority'])  # [71] приоритет скважины в пределах радиуса priority_radius
