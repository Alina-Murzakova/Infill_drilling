...
def load_frac_info(path_frac, data_wells, name_object, dict_parameters_coefficients):
    ...
    """
    if dict_parameters_coefficients['switches']['switch_fracList_params']:
        ...
        # Подготовка файла
        data_frac['well_number'] = data_frac[
            'well_number'].ffill()  # протягивание номера скважины в объединенных ячейках
        ...
        if avg_length_FracStage != 0 and not pd.isna(avg_length_FracStage):
            dict_parameters_coefficients['well_params']["fracturing"]['length_FracStage'] = round(avg_length_FracStage,
                                                                                                  0)
        ...
        data_wells['xfr'] = dict_parameters_coefficients['well_params']["fracturing"]['xfr']
        data_wells['w_f'] = dict_parameters_coefficients['well_params']["fracturing"]['w_f']
        type_frac = dict_parameters_coefficients['well_params']["fracturing"]['Type_Frac']
        length_FracStage = dict_parameters_coefficients['well_params']["fracturing"]['length_FracStage']

        data_wells['FracCount'] = data_wells.apply(lambda row: check_FracCount(type_frac=type_frac,
                                                                               length_FracStage=length_FracStage,
                                                                               L=row['length_geo'],
                                                                               well_type=row['well_type']),
                                                   axis=1)
        data_wells['length_FracStage'] = length_FracStage
    return data_wells, dict_parameters_coefficients


def check_FracCount(type_frac, length_FracStage=1, L=1, well_type='vertical'):
    ...
    if type_frac is None:
        ...
        if L == 0 or well_type == 'vertical':
            return 1
        else:
            FracCount = int(L / length_FracStage)
            if FracCount == 0:
                return 1
            else:
                return FracCount
